#pragma once
#include "../Dictionary/qSort.h"
#include "../Dictionary/binarySearch.h"
...
public:
	...
	iterator find(Type elem) const
	{
		if (_is_sorted)
			return iterator(&_data[binary_search(_data, elem, _size)]);
	...
	iterator insert(iterator it, const Type& elem)
	{
		int diff = it - begin();
		push_back(Type());
		//if (_capacity == _size)
		//	resize(_size * 1.5);
		for (int i = _size - 1; i > diff; --i)
		{
			_data[i] = _data[i - 1];
		}
		_data[diff] = elem;
	...
	{
		int diff = it_end - it_beg + 1;
		int idx = it_beg - begin();
		for (int i = idx; i < _size - diff; ++i)
		{
